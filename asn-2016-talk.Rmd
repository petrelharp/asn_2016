...
```{r setup_habitat_one_sigma, echo=FALSE, cache=TRUE}
...
```
```{r habitat_one_sigma, echo=FALSE, cache=TRUE, depends="setup_habitat_one_sigma"}
...
```{r setup_wildfire_spread, echo=FALSE, cache=TRUE}
if (!file.exists("desert_veg_central_small_raster.grd")) {
...
```
```{r setup_demog, echo=FALSE, cache=TRUE}
demog <- demography(
...
```
```{r run_sim, echo=FALSE, cache=TRUE, depends=c("setup_wildfire_spread","setup_habitat_one_sigma")}
...
![spatial-sweep-Low_Elevation_Wash_System.mp4]

------------------

![spatial-sweep-Creosote.mp4]

------------------

```{r show_speeds, cache=TRUE, depends="run_sim", include=FALSE}
...
speeds[[vegtype]] <- with( list2env(wildfire.sims[[vegtype]]), {
        ...
        legend( "bottomright", legend=c(pop$genotypes[-1],names(wildfire.sims)), lty=c(1:2,1,1), 
               col=c("black","black",hcols[names(wildfire.sims)]) )
...
```{r vol_versus_radius, echo=FALSE, cache=TRUE}
...
```{r habitat_speeds, echo=FALSE, cache=TRUE}
...
Naive prediction of speed is $\sigma \sqrt{2s} = 180$m/gen.
...
legend("topleft",pch=20,col=seq_along(levels(speeds$habitat)), legend=levels(speeds$habitat) )
```
