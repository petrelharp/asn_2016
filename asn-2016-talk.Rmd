...
pestabs.ld <- data.frame( habitat=rep(names(low.density.establishment.probs),sapply(low.density.establishment.probs,nrow)),
        do.call( rbind, low.density.establishment.probs ) )
pestabs.ld$habitat <- factor( pestabs.ld$habitat, levels=levels(pestabs$habitat) )
par(mar=c(4,4,1,1)+.1)
plot(pestabs$s, pestabs$p_estab, col=pestabs$habitat, xlab="s", ylab="prob of establishment", pch=20)
 points(pestabs.ld$s, pestabs.ld$p_estab, col=pestabs.ld$habitat )
...
for (k in seq_along(establishment.probs)) {
    ...
    abline(coef(lm(p_estab ~ s, data=data.frame(establishment.probs[[k]]))), col=k)
      abline(coef(lm(p_estab ~ s, data=data.frame(low.density.establishment.probs[[k]]))), col=k)
}
# sort( with( pestabs, tapply( p_estab/s, habitat, mean ) ) )
```
